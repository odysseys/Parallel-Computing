...
/*
    //SIMD-like
    //#pragma omp parallel for reduction(+: dist1, dist2, dist3, dist4)
    for (i = 0; i < numdims; i+=4)
    {
...
*/    
    ...
    clusters[0] = (float*)  malloc(numClusters * reallo_numCoords * sizeof(float));
    assert(clusters[0] != NULL);
    for (i=1; i<numClusters; i++)
        clusters[i] = clusters[i-1] + reallo_numCoords;
...
/*
    ...
    for(i = 1; i < numClusters; i++)
    {
        ...
        for(k = 0; k < numObjs; k++)
        {
            idx = find_nearest_cluster(i, reallo_numCoords, reallo_objs[k], clusters);
            nearest_dist[k] = euclid_dist_2(reallo_numCoords, reallo_objs[k], clusters[idx]);
        }
        ...
        for(j = 0; j < reallo_numCoords; j++)
        {
            clusters[i][j] = reallo_objs[next_centroid][j];
        }
...
*/
    ...
    newClusters[0] = (float*)  calloc(numClusters * reallo_numCoords, sizeof(float));
    assert(newClusters[0] != NULL);
    for (i=1; i<numClusters; i++)
        newClusters[i] = newClusters[i-1] + reallo_numCoords;
    ...
    if (!is_perform_atomic) {
        ...
        for (i=0; i<nthreads; i++) {
            for (j=0; j<numClusters; j++) {
                local_newClusters[i][j] = (float*)calloc(reallo_numCoords,
                                                         sizeof(float));
    ...
    do {
        ...
        if (is_perform_atomic) {
            #pragma omp parallel for \
                    reduction(+:delta)
                    //private(i,j,index) \
                    //firstprivate(numObjs,numClusters,reallo_numCoords) \
                    //shared(reallo_objs,clusters,membership,newClusters,newClusterSize) \
                    //schedule(static) \
                    //reduction(+:delta)
            for (i=0; i<numObjs; i++) {
                /* find the array index of nestest cluster center */
                index = find_nearest_cluster(numClusters, reallo_numCoords, reallo_objs[i],
                                             clusters);
                ...
                for (j=0; j<reallo_numCoords; j++)
                    #pragma omp atomic
                    newClusters[index][j] += reallo_objs[i][j];
            }
        }
        else {
            #pragma omp parallel \
                    shared(reallo_objs,clusters,membership,local_newClusters,local_newClusterSize)  
            {
                ...
                for (i=0; i<numObjs; i++) {
                    /* find the array index of nestest cluster center */
                    index = find_nearest_cluster(numClusters, reallo_numCoords,
                                                 reallo_objs[i], clusters);
                    ...
                    for (j=0; j<reallo_numCoords; j++)
                        local_newClusters[tid][index][j] += reallo_objs[i][j];
                }
