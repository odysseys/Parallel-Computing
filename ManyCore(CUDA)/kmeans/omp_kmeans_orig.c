...
#pragma omp parallel for schedule(static)
    {
        ...
        int nthrds = omp_get_num_thread();
        for (i = 1 +id; i < numClusters; i+nthrds) {
            //clusters[i] = clusters[i - 1] + numCoords;
            clusters[i] = i * numCoords + clusters[0];
        }
    }
#pragma omp parallel for schedule(static)
    {
        ...
        for (i=id; i<numClusters; i+nthrds)
            for (j=0; j<numCoords; j++)
                clusters[i][j] = objects[i][j];
        /* initialize membership[] */
        for (i=id; i<numObjs; i+nthrds) {
            membership[i] = -1;
        }
    }
